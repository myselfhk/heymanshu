...
function FaqItem({ q, a }) {
  ...
  return (
    <div style={{borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
      ...
      >
        <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:15,fontWeight:400,color:'rgba(255,255,255,0.65)'}}>{q}</span>
        <span style={{color:'rgba(255,255,255,0.3)',fontSize:18,flexShrink:0,transform:open?'rotate(45deg)':'rotate(0)',transition:'transform 0.3s'}}>+</span>
      </button>
      <div style={{
        overflow:'hidden', maxHeight: open ? 200 : 0,
        transition:'max-height 0.4s cubic-bezier(0.22,1,0.36,1)',
      }}>
        <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:300,color:'rgba(255,255,255,0.4)',lineHeight:1.7,paddingBottom:16}}>{a}</p>
      </div>
    </div>
  )
}
...
export default function ShelfPage() {
  ...
  return (
    <div ref={pageRef} style={{ background:'#F7F4EF', minHeight:'100vh', fontFamily:"'DM Sans',sans-serif", overflowX:'hidden' }}>
      ...
      <div style={{ padding:'160px 80px 64px', borderBottom:'1px solid rgba(0,0,0,0.06)' }}>
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:11,fontWeight:600,letterSpacing:'0.18em',textTransform:'uppercase',color:'rgba(0,0,0,0.3)',marginBottom:16}}>The Shelf</div>
        <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(48px,6vw,80px)',fontWeight:300,lineHeight:0.95,letterSpacing:'-0.03em',color:'#0E0E0E',marginBottom:20}}>
          Things I made<br /><em>that you can use.</em>
        </h1>
        <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:16,fontWeight:300,color:'rgba(0,0,0,0.45)',maxWidth:'48ch',lineHeight:1.7}}>
      ...
      {PRODUCTS.map((product, i) => (
        ...
        >
          ...
          {i % 2 !== 0 && (
            <div style={{
              ...
              <div style={{position:'relative',zIndex:1}}>
                <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:80,fontWeight:300,color:'rgba(255,255,255,0.06)',lineHeight:1,display:'block',marginBottom:16}}>{product.num}</span>
                <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:32,fontWeight:300,color:'white',lineHeight:1.1,marginBottom:8}}>{product.name}</h3>
                <p style={{fontFamily:"'Syne',sans-serif",fontSize:10,letterSpacing:'0.14em',textTransform:'uppercase',color:product.accentColor}}>{product.meta}</p>
                <div style={{marginTop:32,fontFamily:"'Cormorant Garamond',serif",fontSize:40,fontWeight:300,color:'white'}}>{product.price}</div>
              </div>
            </div>
          )}
          ...
          {/* Product text */}
          <div>
            <span style={{fontFamily:"'Syne',sans-serif",fontSize:10,fontWeight:600,letterSpacing:'0.16em',textTransform:'uppercase',color:'rgba(0,0,0,0.3)'}}>{product.num}</span>
            <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'clamp(28px,3vw,44px)',fontWeight:300,lineHeight:1.1,color:'#0E0E0E',margin:'12px 0 16px',letterSpacing:'-0.02em'}}>{product.name}</h2>
            <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:16,fontWeight:300,color:'rgba(0,0,0,0.55)',lineHeight:1.7,marginBottom:32,maxWidth:'52ch'}}>{product.description}</p>

            {/* What's included */}
            <div style={{marginBottom:28}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:10,letterSpacing:'0.16em',textTransform:'uppercase',color:'rgba(0,0,0,0.35)',marginBottom:12}}>What's included</div>
              <ul style={{listStyle:'none',padding:0,display:'flex',flexDirection:'column',gap:8}}>
                {product.included.map((item, j) => (
                  <li key={j} style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,color:'rgba(0,0,0,0.6)',display:'flex',gap:10,alignItems:'flex-start'}}>
            ...
            <div style={{marginBottom:32}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:10,letterSpacing:'0.16em',textTransform:'uppercase',color:'rgba(0,0,0,0.35)',marginBottom:8}}>Who it's for</div>
              <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:14,color:'rgba(0,0,0,0.55)',lineHeight:1.65}}>{product.forWho}</p>
            </div>

            {/* FAQ */}
            <div style={{marginBottom:36}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:10,letterSpacing:'0.16em',textTransform:'uppercase',color:'rgba(0,0,0,0.35)',marginBottom:8}}>FAQ</div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:20}}>
              ...
              <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:15,fontWeight:500,color:'#0E0E0E'}}>{product.price}</span>
            </div>
          </div>
      ...
      <div style={{padding:'80px 80px 120px'}}>
        <p style={{fontFamily:"'DM Sans',sans-serif",fontSize:15,color:'rgba(0,0,0,0.3)'}}>More tools in the making.</p>
      </div>
    </div>
  )
}
