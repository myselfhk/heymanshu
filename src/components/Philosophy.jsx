...
const DEFAULT_SENTENCE = "Design is not what something looks like. It's what decision it protects you from making."
...
export default function Philosophy() {
  ...
  const sentence = settings?.philosophyQuote || DEFAULT_SENTENCE
  const words    = sentence.split(' ')

  useEffect(() => {
    const wordEls = quoteRef.current?.querySelectorAll('.word')
    ...
    mm.add('(min-width: 768px)', () => {
      ...
      gsap.set(wordEls,         { color: 'rgba(255,255,255,0.12)' })
    ...
    mm.add('(max-width: 767px)', () => {
      ...
      gsap.set(wordEls, { color: 'rgba(255,255,255,1)' })
      gsap.set(attrRef.current, { opacity: 0, y: 12 })
